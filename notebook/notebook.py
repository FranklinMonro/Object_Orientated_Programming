...
class Note:
    ...
    def match(self, filter):
        '''Determine if this note matches the filter text.
            Return true if it maches, otherwise false.
...
class Notebook:
    ...
    def modify_memo(self, note_id, memo):
        # Find the note with the given id and change memo
        note = self._fin_note(note_id)
        if note:
            note.memo = memo
            return True
        return False
    ...
    def modify_tags(self, note_id, tags):
        # Find the note with the give id and change tag
        self._find_note(note_id).tags = tag
